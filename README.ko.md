...
| OS | 파일 | 설치 방법 |
|---|---|---|
| **Windows 10/11** | [`CaptureIt-Setup-2.1.10-win-x64.exe`](https://github.com/th00tames1/CaptureIt/releases/download/v2.1.10/CaptureIt-Setup-2.1.10-win-x64.exe) | 설치 관리자를 실행합니다. 관리자 권한이 필요하지 않습니다. |
| Windows (무설치) | [`CaptureIt-Portable-2.1.10-win-x64.zip`](https://github.com/th00tames1/CaptureIt/releases/download/v2.1.10/CaptureIt-Portable-2.1.10-win-x64.zip) | 압축 풀고 exe 실행 |
| **Linux x64** | [`CaptureIt-2.1.10-linux-x64.tar.gz`](https://github.com/th00tames1/CaptureIt/releases/download/v2.1.10/CaptureIt-2.1.10-linux-x64.tar.gz) | 압축 해제 후 `./install.sh` |
| **macOS (Apple Silicon)** | [`CaptureIt-2.1.10-macos-apple-silicon.zip`](https://github.com/th00tames1/CaptureIt/releases/download/v2.1.10/CaptureIt-2.1.10-macos-apple-silicon.zip) | 압축 해제 → **우클릭 → 열기** |
| macOS (Intel) | [`CaptureIt-2.1.10-macos-intel.zip`](https://github.com/th00tames1/CaptureIt/releases/download/v2.1.10/CaptureIt-2.1.10-macos-intel.zip) | 압축 해제 → **우클릭 → 열기** |
...
| **Print Screen** | `PrtSc` 키로 영역 캡처를 시작합니다. 기본 해제, 설정 → 단축키에서 켜기 | `PrtSc` |

> **Windows 11의 Print Screen**: Windows가 `PrtSc`를 캡처 도구(Snipping Tool)에 먼저 연결해 두면
> 어떤 앱보다 우선합니다. CaptureIt이 이를 감지해 해제 방법(설정 → 접근성 → 키보드)을 안내합니다.
> Windows 설정 값을 앱이 직접 바꾸지는 않습니다.
>
> **혹시 멈춘 것 같으면** (오버레이가 안 닫히거나 단축키가 안 먹힐 때) `Ctrl+Alt+Shift+Q` 를 누르거나
> 트레이 메뉴의 *캡처 취소하고 창 열기* 를 선택하면 진행 중이던 캡처를 취소하고 메인 창을 되살립니다.
...
- 편집기의 **메인 창** 버튼(`Ctrl+M`)으로 캡처 툴바를 언제든 다시 불러오기
- 인앱 업데이트: 새 버전이 나오면 팝업으로 알리고 한 번의 클릭으로 설치 (설정 → 업데이트에서 언제든 확인)
